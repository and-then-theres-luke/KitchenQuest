const ingredient_lookup = async(table, search_string) => {
    ...
    // Clear the data
    tableBody.innerHTML = "";
    for (const row of results) { // Note here how we are defining a variable row that gets referenced again below, we are doing something for each something for each something in a row.
        const rowElement = document.createElement("tr"); // This CREATES the row
            ...
            imageCellImage.src = "https://spoonacular.com/cdn/ingredients_100x100/" + row.image
            ...
            actionsAddToPantry.href = "/ingredient/show_one/" + row.id
...
const recipe_lookup_by_ingredients = async(table, search_string) => {
